...
\ifans{For the single pair of words $c$ and $o$, $\by$ is one-hot, being $1$ for $o$ and $0$ everywhere else.

(Note that cross the whole dataset, a center word $c$ would have different outside words $o$.)}
...
\begin{enumerate}[label=\roman*.]
    ...
    \begin{align*} 
    	...
    	&= -\log P(O=o| C=c) \\
    	&= -\log \frac{\exp(\bu_{o}^\top \bv_c)}{\sum_{w \in \text{Vocab}} \exp(\bu_{w}^\top \bv_c)} \\
    	&= -\log \exp(\bu_{o}^\top \bv_c) + \log \sum_{w \in \text{Vocab}} \exp(\bu_{w}^\top \bv_c) \\
    	&= - \bu_{o}^\top \bv_c + \log \sum_{w \in \text{Vocab}} \exp(\bu_{w}^\top \bv_c) \\
    	\frac{\partial \bJ_{\text{naive-softmax}}(\bv_c, o, \bU)}{\partial \bv_c}
    	&= -\frac{\partial \bu_{o}^\top \bv_c}{\partial \bv_c} + \frac{\partial \log \sum_{w \in \text{Vocab}} \exp(\bu_{w}^\top \bv_c)}{\partial \bv_c} \\
    	&= -\bu_{o}^\top + \frac{\sum_{w \in \text{Vocab}} \bu_{w}^\top \exp(\bu_{w}^\top \bv_c)}{\sum_{w \in \text{Vocab}} \exp(\bu_{w}^\top \bv_c)} \\
    	&= -\bu_{o}^\top + \sum_{w \in \text{Vocab}} \bu_{w}^\top \hat{\by}_w \\
    	&= -\sum_{w \in \text{Vocab}} \bu_{w}^\top \by_w + \sum_{w \in \text{Vocab}} \bu_{w}^\top \hat{\by}_w \\
    	&= -\by^\top\bU^\top  + \hat{\by}^\top\bU^\top \\
    	&= (\hat{\by}^\top - \by^\top)\bU^\top \tag{cf. textbook Eq. 5.48}
   	\end{align*}
    }
    \item
    ...
    \ifans{
    \begin{align*}
    	(\hat{\by}^\top - \by^\top)\bU^\top = 0 &\Longleftrightarrow \bU(\hat{\by} - \by) = 0 \\
    	&\Longleftrightarrow (\hat\by-\by) \in \text{Ker}\ \bU\ \text{(or}\ \text{Null}\ \bU) \\
    	&\Longleftrightarrow (\hat\by-\by)\ \text{is orthogonal to}\ \text{Ran}\ \bU
    \end{align*}
	}
    \item
    ...
    \ifans{ When the gradient is subtracted, it becomes $\by^\top\bU^\top - \hat{\by}^\top\bU^\top$. The first term moves $\bv_c$ towards the true ‘outside’ vector $\bu_o$ closer by adding $\bu_{o}^\top\by_o$, while the second term moves $\bv_c$ farther away from noise words by subtracting $\bu_{w}^\top\hat{\by}_w (w\neq o)$. (Note that the second term ‘lags’ the first term by subtracting $\bu_{o}^\top\hat{\by}_o$)
    }
...
\begin{align*}
	\bu_x = \alpha\bu_y
	&\implies ||\bu_x|| = \alpha||\bu_y|| \\
	&\implies \bu_x/||\bu_x|| = \alpha\bu_y/(\alpha||\bu_y||) = \bu_y/||\bu_y||
	\tag{\alpha\ >\ 0}
\end{align*}
Then we cannot distinguish the contribution to sum between $\bu_x$ and $\bu_y$ after normalization. For example, $\bu_x/||\bu_x|| + \bu_z/||\bu_z||$ and $\bu_y/||\bu_y|| + \bu_z/||\bu_z||$ will be the same. And if $||\bu_x||$ is \gg $||\bu_y||$, $\bu_x + \bu_z$ should be much ‘closer’ to $\bu_x$ than $\bu_y + \bu_z$ to $\bu_y$, which just cannot be reflected in the sum after normalization.

Another example, if we have $\bx = (3, 4)$ and $\by = (-6, -8)$, before normalization we have $\bx + \by = (-3, -4)$, which just isn't the case, since $\bx/||\bx|| + \by/||\by|| = (0, 0)$, we eliminate both its magnitude and direction.

The only occasion for me when it doesn't take away useful information, is when all embeddings are of almost same size. But I don't see the point normalizing then. Therefore when it comes to this specific task, I'd rather not do the normalization.  

Related: \href{https://stackoverflow.com/questions/62827526/what-do-negative-vectors-mean-on-word2vec}{What do negative vectors mean on word2vec?}
}
...
\ifans{
\begin{align*} 
	\frac{\partial \bJ_{\text{naive-softmax}}(\bv_c, o, \bU)}{\partial \bu_o}
	&= -\frac{\partial \bu_{o}^\top \bv_c}{\partial \bu_o} + \frac{\partial \log \sum_{w \in \text{Vocab}} \exp(\bu_{w}^\top \bv_c)}{\partial \bu_o} \\
	&= -\bv_c^\top + \frac{ \bv_{c}^\top \exp(\bu_{o}^\top \bv_c)}{\sum_{w \in \text{Vocab}} \exp(\bu_{w}^\top \bv_c)} \\
	&= -\bv_c^\top + \bv_c^\top\hat{\by}_o \\
	\frac{\partial \bJ_{\text{naive-softmax}}(\bv_c, o, \bU)}{\partial \bu_i}
	&= \frac{\partial \log \sum_{w \in \text{Vocab}} \exp(\bu_{w}^\top \bv_c)}{\partial \bu_i} \\
	&= \frac{\bv_{c}^\top \exp(\bu_{i}^\top \bv_c)}{\sum_{w \in \text{Vocab}} \exp(\bu_{w}^\top \bv_c)} \\
	&= \bv_c^\top\hat{\by}_i \tag{i\ \neq\ o}
\end{align*}

Related: \href{https://math.stackexchange.com/questions/20694/vector-derivative-w-r-t-its-transpose-fracdaxdxt}{Vector derivative w.r.t its transpose $\frac{d(Ax)}{d(x^T)}$}
}
% Question 1-E
\part[1]

...
\begin{enumerate}[label=\roman*.]
    ...
    \begin{align*} 
    	...
    	&= -\bU^\top \by + \bU^\top\hat{\by} \\
    	&= \bU^\top (\hat{\by} - \by) \tag{cf. textbook Eq. 5.48}
   	\end{align*}
    }
    \item
    ...
    \ifans{
    \begin{itemize}
    	\item $(\hat\by-\by) \in \text{Ker}\ \bU^\top$ (or $\text{Null}\ \bU^\top$)
    	\item $(\hat\by-\by)$ is orthogonal to $\text{Ran}\ \bU^\top$
    \end{itemize}
	}
    \item
    ...
    \ifans{ When the gradient is subtracted, it becomes $\bU^\top \by - \bU^\top\hat{\by}$. The first term moves $\bv_c$ towards the true ‘outside’ vector $\bu_o$ closer by adding $\bu_{o}^\top\by_o$, while the second term moves $\bv_c$ farther away from noise words by subtracting $\bu_{w}^\top\hat{\by}_w (w\neq o)$. (Note that the second term ‘lags’ the first term by subtracting $\bu_{o}^\top\hat{\by}_o$)
    }
